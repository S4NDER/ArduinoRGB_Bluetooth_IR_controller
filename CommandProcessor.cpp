...
void CommandProcessor::processCommand (void){
    if (controller.getPowerState() || (inputCommand == IR_ON)) {
        switch (inputCommand) {
            ...
            case IR_BRIGHT_UP: effect.bright_up(color); inputCommand = 0; break;
            case IR_BRIGHT_DOWN: effect.bright_down(color); inputCommand = 0; break;
            case IR_OFF: controller.turn_off(); controller.set_led(color); break;
            case IR_ON: controller.turn_on(); controller.set_led(color); break;
            case IR_STROBE: effect.flash(color, 200); break;
            case MIC_FLASH: controller.turn_off(); controller.set_led(color); delay(25); controller.turn_on(); controller.set_led(color); Serial.println("Did i broke it?"); inputCommand = BT_COMMAND; break;
            case IR_FLASH:
                switch (flashCounter) {
